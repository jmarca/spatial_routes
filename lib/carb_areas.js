...
// ------------+-----------------------+----------------------------------------------------------------
//  gid        | integer               | not null default nextval('carbgrid.state4k_gid_seq'::regclass)
//  fid_state4 | integer               |
...
//  geom4326   | geometry              |
// Indexes:
//     "state4k_pkey" PRIMARY KEY, btree (gid)
//     "state4k_geom_gist" gist (geom)
// Check constraints:
//     "enforce_dims_geom" CHECK (st_ndims(geom) = 2)
//     "enforce_dims_geom4326" CHECK (st_ndims(geom4326) = 2)
//     "enforce_geotype_geom" CHECK (geometrytype(geom) = 'MULTIPOLYGON'::text OR geom IS NULL)
//     "enforce_geotype_geom4326" CHECK (geometrytype(geom4326) = 'MULTIPOLYGON'::text OR geom4326 IS NULL)
//     "enforce_srid_geom" CHECK (st_srid(geom) = 10000)
//     "enforce_srid_geom4326" CHECK (st_srid(geom4326) = 4326)

    var grid4k_handler = shape_service({'db':'spatialvds'
                          ,'table':'carbgrid.state4k'
                          ,'alias':'state4k'
                          ,'host':phost
                          ,'port':pport
